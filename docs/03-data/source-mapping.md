...
| Opening odds | Desirable odds reference (optional, not mandatory MVP) | Odds | No | Football-Data.co.uk / TheStatsAPI / source-specific | Unknown / source-dependent | Direct (when available) | Partially verified | Historical opening odds are explicitly referenced by Football-Data.co.uk and TheStatsAPI | Coverage varies by provider | Opening odds must not be conflated with closing or early snapshot odds; must not be inferred from first snapshot | Can be bookmaker-specific or aggregated, depending on source. Required decision per Provider Selection Section 2.1: NOT mandatory MVP, but desirable when reliably available |
